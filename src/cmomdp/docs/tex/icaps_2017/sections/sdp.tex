...
A case statement can be maximized with respect to a continuous parameter {\footnotesize$y$} as {\footnotesize $ f_1(\vec{x}) = \contmax_{y}f_2(\vec{x}, y) $}. The continuous maximization operation is used for continuous actions in PHMDPs and remains closed-form for LPWL functions; maximization of discrete actions remains closed-form for all NPWN functions. We refer the reader to~\cite{Sanner_UAI_2011, Zamani_AAAI_2012} for further details.
...
The PHMDP framework permits any variant of the inverse learning problem for multi-objective MDPs. We note that the formulation above is but one of many possible approaches to inverse reinforcement learning and refer the reader to~\cite{Ng_ICML_2000} for alternate approaches. 
